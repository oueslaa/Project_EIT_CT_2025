classdef MeshBuilder
    ...
    methods(Static)
        function obj = from_polys(shapes, domain, ext_smooth, z_slice, params)
            ...
            P_ext = ext_smooth;
            dP    = [inf; hypot(diff(P_ext(:,1)), diff(P_ext(:,2)))]; P_ext(dP<tol,:) = [];
            ...
            cleanContours = {P_ext}; groupList = {'SoftTissue'};

            % 2) Organes par régions
            orgFields = fieldnames(shapes);
            for k = 1:numel(orgFields)
                shp = shapes.(orgFields{k}); if shp.NumRegions == 0, continue; end
            ...
            % 3) PDE geometry
            nS = numel(cleanContours); vk = cellfun(@(C) size(C,1), cleanContours); maxk = max(vk);
            gd = zeros(2 + 2*maxk, nS);
            for i = 1:nS
                P = cleanContours{i}; k = size(P,1);
                gd(1,i) = 2; gd(2,i) = k; gd(3:2+k,i) = P(:,1); gd(3+k:2+2*k,i) = P(:,2);
            end
            ...
            % 4) Mesh (transition de taille douce)
            msh = generateMesh(model, 'Hmax', params.targetSize, 'Hgrad', 1.4, ...
                               'GeometricOrder','linear', 'Jiggle',{'on','minimum'});
            g = msh.Nodes'; H = msh.Elements'; contour = P_ext;

            % 5) triGroup par barycentre
            centroids = ( g(H(:,1),:)+g(H(:,2),:)+g(H(:,3),:) )/3;
            M = size(H,1); triGroup = zeros(M,1);
            for t = 1:M
                pt = centroids(t,:); assigned = false;
                for k = 2:numel(types)
                    if isfield(shapes, types{k}) && isinterior(shapes.(types{k}), pt(1), pt(2))
                        triGroup(t) = k; assigned = true; break; end
                end
                if ~assigned && isinterior(domain, pt(1), pt(2)), triGroup(t) = 1; end
            end

            obj = MeshBuilder; obj.g=g; obj.H=H; obj.triGroup=triGroup; obj.shapes=shapes;
            obj.domain=domain; obj.groups=types; obj.group_names=names; obj.contour=contour;
        end
    end 
   methods 

        function saveMesh(obj, outdir, z_slice)
            outdir = char(outdir); % ✅ conversion string → char
            if ~exist(outdir, 'dir')
                mkdir(outdir);
            end
            fname = fullfile(outdir, sprintf('mesh_slice%03d.mat', z_slice));
            g = obj.g; H = obj.H; triGroup = obj.triGroup; shapes = obj.shapes; domain = obj.domain;
            save(fname, 'g', 'H', 'triGroup', 'shapes', 'domain');
        ...
        function show(obj)
            ...
            for k = 1:numel(obj.groups)
                idx = (obj.triGroup == k); if ~any(idx), continue; end
                tris = obj.H(idx,:);
                patch('Faces', tris, 'Vertices', obj.g, 'FaceColor', cmap(k,:), 'FaceAlpha', 0.85, 'EdgeColor', 'none');
            end
            P = obj.contour; if ~isequal(P(1,:), P(end,:)), P=[P;P(1,:)]; end
            plot(P(:,1), P(:,2), 'r-', 'LineWidth', 1.5);
            axis equal tight; title('Mesh et groupes'); legend(obj.group_names, 'Location','BestOutside');
        end
    end
end