...
% - reg      : struct('tik_weight',..,'W',..) pour la régularisation sur p
    ...
    mU = numel(U0);
    if numel(y) ~= mU
        if numel(y) > mU, y = y(1:mU);
        else, k = ceil(mU/numel(y)); y = repmat(y, k, 1); y = y(1:mU);
        end
    end
    ...
    for k = 1:K
        ...
        if isempty(sk), p0(k) = obj.get_cond('SoftTissue'); else, p0(k) = mean(sk); end
    end

    % ---- bornes & régularisation ----
    lb = 0.02;  if isfield(reg,'lb') && ~isempty(reg.lb), lb = reg.lb; end
    ub = 1.50;  if isfield(reg,'ub') && ~isempty(reg.ub), ub = reg.ub; end
    p0 = min(max(p0, lb), ub);

   % ---------- régularisation ----------
if ~isfield(reg,'tik_weight') || isempty(reg.tik_weight), reg.tik_weight = 1e-2; end
reg.p0 = p0;
...
% --- flags d'affichage
flag = double(~isempty(doPlots) && any(doPlots(:)));
invSolver.plotLinesearch  = flag;
invSolver.plotIterations  = flag;
invSolver.plotData        = 0;
invSolver.plotConvergence = flag;
invSolver.plotUQ          = 0;
...
function plot_all(obj, exportBase, mode, opts)
    ...
    % ⚠️ NE PAS mettre "end, end" sur une seule ligne (cause l'erreur)
    vis = 'on';
    if ~showFigures
        vis = 'off';
...
if opts.pdf
    ...
    % -- helper d'affichage (accepte Nodal ou Triangulaire)
    ...
    set(ax,'XTick',[],'YTick',[]); box(ax,'on'); title(ax,'Mesh + electrodes','Interpreter','none');
    try, hide_axes_toolbar(ax); end %#ok<TRYNC>
    apply_display_convention(ax, mode);
    ...
    sigFwd = [];
    if isprop(obj,'sigma_tri'), sigFwd = obj.sigma_tri; end
    if isempty(sigFwd) || numel(sigFwd) ~= size(obj.H,1)
        if ismethod(obj,'sigma_tri_from_groups')
            ...
            for kk = 1:numel(obj.groups)
                ...
                if isfield(obj.cond,name)
                    sigFwd(idx) = obj.cond.(name);
                else
                    sigFwd(idx) = obj.cond.SoftTissue;
                end
            end
        end
    end
    ...
    grid(ax,'on'); xlabel(ax,'Measurement index'); ylabel(ax,'Voltage [V]');
    title(ax,'Simulated signals','Interpreter','none');
    do_export(fig, 'signals');

    % --- helper local tri->noeud ---
